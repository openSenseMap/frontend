...
async function seed() {
  ...
  const devices = await csvtojson().fromFile("./drizzle/seeds/devices.csv");
  let i = 0;
  ...
  const sensors = await csvtojson().fromFile("./drizzle/seeds/sensors.csv");
  ...
  try {
    const measurements = await csvtojson().fromFile(
      "./drizzle/seeds/measurements.csv"
    );
    let k = 0;
