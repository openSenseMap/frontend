...
ALTER TABLE "location" ADD COLUMN "id_bigserial" bigserial;

-- Copy old id values
UPDATE location SET id_bigserial = id;
UPDATE device_to_location SET location_id_bigint = location_id;
UPDATE measurement SET location_id_bigint = location_id;

-- Update last_value of sequence
SELECT setval('location_id_bigserial_seq', COALESCE(MAX(id), 1)) FROM location;
