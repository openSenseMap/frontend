...
import { type DeviceExposureType, type DeviceStatusType } from "~/db/schema/enum";
...
function normalizeToggleValues<T extends string>(values: string[], currentValues: T[]): T[] {
  if (values.length === 0) {
    return [];
  }
  ...
  return (
    <div className="flex flex-col gap-3">
      <div className="grid gap-1.5 md:grid-cols-[5.5rem_1fr] md:items-center">
        <Label className="text-sm text-gray-600 dark:text-zinc-400">{t("exposure")}</Label>
        ...
        <ToggleGroup
          ...
          onValueChange={(values) => {
            onExposureChange(normalizeToggleValues<DeviceExposureType>(values, exposure));
          }}
        >
      ...
      <div className="grid gap-1.5 md:grid-cols-[5.5rem_1fr] md:items-center">
        <Label className="text-sm text-gray-600 dark:text-zinc-400">{t("status")}</Label>
        ...
        <ToggleGroup
          ...
          onValueChange={(values) => {
            onStatusChange(normalizeToggleValues<DeviceStatusType>(values, status));
          }}
        >
          <ToggleGroupItem value="all" aria-label="Toggle all" size="sm">
            {t("all")}
          ...
          <ToggleGroupItem value="inactive" aria-label="Toggle inactive" size="sm">
            {t("inactive")}
          </ToggleGroupItem>
