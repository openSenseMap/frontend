...
/**
 * Query measurements for a set of sensorIds.
 * - When bbox is provided, include measurements with no location.
 * - Left-join the location table so measurement rows may include measurement.location.
 */
export async function queryMeasurements(
	...
	if (sensorIds.length === 0) return { measurements: [], locationsMap: {} }

	const conditions = [
		inArray(measurement.sensorId, sensorIds),
		between(measurement.time, fromDate, toDate),
	]

	let bboxConditionSql = undefined

	if (bbox) {
		const [lonSW, latSW, lonNE, latNE] = bbox

		bboxConditionSql = sql`
      (
        ${location.location} IS NULL
        OR ST_Within(
            ${location.location},
            ST_MakeEnvelope(${lonSW}, ${latSW}, ${lonNE}, ${latNE}, 4326)
          )
      )
    `
	}

	const whereClause = bboxConditionSql
		? and(...conditions, bboxConditionSql)
		: and(...conditions)
	...
	const rows = await drizzleClient
		...
		.from(measurement)
		.leftJoin(location, eq(location.id, measurement.locationId))
		.where(whereClause)
	...
	if (locationIds.length > 0) {
		const locRows = await drizzleClient
			.select({
				id: location.id,
		...
		for (const loc of locRows) {
			if (loc.location) {
				locationsMap[loc.id.toString()] = {
					lon: (loc.location as any).x,
					lat: (loc.location as any).y,
				}
			}
		}
	}

	return { measurements: rows, locationsMap }
}
