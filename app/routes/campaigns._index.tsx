...
import { CampaignOverviewMap } from '~/components/campaigns/campaign-overview-map'
...
export async function loader({ request }: Route.LoaderArgs) {
	...
	const rawBookmarks = url.searchParams.get('bookmarks') || undefined
	...
	const bookmarkFilter =
		rawBookmarks === 'bookmarked' ? rawBookmarks : undefined
	const sort =
		rawSort === 'starting' || rawSort === 'bookmarks' ? rawSort : 'newest'
	...
	return (
		<div className="min-h-screen bg-slate-50">
			...
			<main className="mx-auto flex w-full max-w-6xl flex-col gap-8 px-4 py-8">
				...
				<Form
					method="get"
					className="grid gap-4 rounded-lg border border-slate-200 bg-white p-4 md:grid-cols-[1fr_220px_180px] lg:grid-cols-[1fr_190px_160px_180px_180px_auto]"
				>
					...
					<div className="space-y-2">
						<Label htmlFor="bookmarks">{t('bookmarks')}</Label>
						<Select name="bookmarks" defaultValue={filters.bookmarks}>
							<SelectTrigger id="bookmarks">
								<SelectValue />
							</SelectTrigger>
							<SelectContent>
								<SelectItem value="all">{t('all_campaigns')}</SelectItem>
								<SelectItem value="bookmarked">
									{t('bookmarked_campaigns')}
								</SelectItem>
							</SelectContent>
						</Select>
					</div>
					<div className="space-y-2">
						...
						<Select name="sort" defaultValue={filters.sort}>
							...
							<SelectContent>
								...
								<SelectItem value="starting">{t('sort_starting')}</SelectItem>
								<SelectItem value="bookmarks">{t('sort_bookmarks')}</SelectItem>
							</SelectContent>
						</Select>
					</div>
				...
				{campaigns.length > 0 ? (
					<>
						...
						>
							{() => <CampaignOverviewMap campaigns={campaigns} />}
						</ClientOnly>
